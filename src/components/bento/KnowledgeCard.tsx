...
export default function KnowledgeCard() {
  // Hardcoded for now until connected to Cloudflare D1
      ...
      <div className="flex flex-col gap-3">
        {latestSnippets.map((item, i) => (
          <Link key={i} to="/knowledge" className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/50 cursor-pointer transition-colors group/item">
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300 group-hover/item:text-blue-600 dark:group-hover/item:text-blue-400">
              {item.title}
            </span>
